...
np = 8
TauDEM_dir = '/home/ahmet/ciwater/usu_data_service/topnet_data_service/TauM/TauDEM-5.3'
...
def watershed_delineation(DEM_Raster,Outlet_shapefile,Src_threshold,Min_threshold,Max_threshold,Number_threshold,
    ...
    #retDictionary = call_subprocess("/usr/local/bin/ogr2ogr -a_srs -overwrite "+projFileSource+" "+'LoganRivermoveOutlet21.shp'+ " "+ 'LoganRivermoveOutlet2.shp', 'copy projection2')
    if retDictionary['success'] == "False":
        return retDictionary

    prj_file='LoganRiverOutletProj.prj'
    in_shp='LoganRivermoveOutlet2.shp'
    out_shp='dummyfile.shp'
    proj_cmd='/usr/local/bin/ogr2ogr -a_srs'+ ' '+ 'LoganRiverOutletProj.prj' + ' '+ 'dummyfile.shp' +' ' + 'LoganRivermoveOutlet2.shp'
    #proj_cmd='/usr/local/bin/ogr2ogr -a_srs'+ ' '+ in_prj + ' '+ out_shp +' ' + in_shp
    #retDictionary = call_subprocess("cp "+projFileSource+" "+projFile, 'copy projection')
    #retDictionary1 = subprocess.call('ogr2ogr')
    ogr_dir=MPI_dir
    retDictionary =call_subprocess(Prj(ogr_dir, In_Out_dir, prj_file, out_shp, in_shp),'projet')
    if retDictionary['success'] == "False":
        return retDictionary
    source = ogr.Open('dummyfile.shp', update=True)
...
# Close the Shapefile
    ...
    if retDictionary['success'] == "False":
        return retDictionary
    retDictionary=call_subprocess(ACCUMULATING_STREAM_SOURCE(MPI_dir,np,TauDEM_dir,In_Out_dir,Input_Dem,out_shp),'aread8')
    if retDictionary['success'] == "False":
        return retDictionary
    retDictionary= call_subprocess(DROP_ANALYSIS(MPI_dir,np,TauDEM_dir,In_Out_dir,Input_Dem,Min_threshold,Max_threshold,Number_threshold,out_shp),'dropana')
    if retDictionary['success'] == "False":
        return retDictionary
    ...
    if retDictionary['success'] == "False":
        return retDictionary
    retDictionary=call_subprocess(StreamNet(MPI_dir,np,TauDEM_dir,In_Out_dir,Input_Dem,out_shp,output_watershedfile,output_treefile, output_coordfile,output_streamnetfile),'delineatewatershed')
    if retDictionary['success'] == "False":
        return retDictionary
...
def Prj(ogr_dir, In_Out_dir, prj_file, out_shp, in_shp):
    commands = []
    # commands.append(os.path.join(MPI_dir,"mpirun"));commands.append("-np");commands.append(str(np))
    commands.append(os.path.join(ogr_dir,"ogr2ogr"))
    commands.append("-a_srs")
    commands.append(os.path.join(In_Out_dir, prj_file));
    commands.append(os.path.join(In_Out_dir, out_shp));
    commands.append(os.path.join(In_Out_dir, in_shp))
...
def daymet_download(Watershed_Raster,Start_Year,End_Year,output_rainfile,output_temperaturefile,output_cliparfile,output_gagefile):
    head,tail=os.path.split(Watershed_Raster)
    watershed_dir= str(head)  #str(os.path.split(str(output_gagefile))[0]   ) # str(head)
    wateshed_name= str(tail)
    #print (Watershed_Raster)
    #print (output_gagefile)
    #print (watershed_dir)
    #print (wateshed_name)
    #
    # head,tail=os.path.split(str(Watershed_Raster))
    ...
    Output_Dir = os.path.dirname(output_rainfile)
  #  os.chdir(watershed_dir)
    ...
    commands=[]
    commands.append("/usr/bin/Rscript");commands.append(daymet_download_script);
    ...
    #print ('Progress --> Started: daymet_download.R .......')
    os.system(fused_command1)
    #print ('Progress --> Ended: daymet_download.R .......')
    time.sleep(40)
    ...
    #print ('Progress --> create_rain.R Completed')
    ...
    #print ('Progress --> create_tmaxtmintdew.R Completed')

    clipar_script= os.path.join(R_Code_Path,'create_clipar.R')
    commands=[]
    commands.append("Rscript");commands.append(clipar_script)
    commands.append(str(Output_Dir));commands.append(str(Start_Year));commands.append(str(End_Year))
