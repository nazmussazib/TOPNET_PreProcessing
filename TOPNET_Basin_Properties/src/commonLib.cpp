...
{
	char temp[1024];
	sprintf(temp,"Numerical recipies run-time error......\n%s",error_text);
...
long *lvector (int nl, int nh)
{
	...
	if(!v) nrerror ("allocation failure in ivetor()");
	return v-nl;
}
...
void **matalloc(int kx, int ky, int datatype)
	{
 int ki, arrsize;
 void **mat;
 void *data;
 ...
 {
	 mat=(void **) malloc (sizeof(short *) *(kx));
	 arrsize=sizeof(short) *(kx)*(ky);
 }

 else if(datatype==2)

 {
	 mat=(void **) malloc(sizeof(int *) *(kx));
	 arrsize=sizeof(int) *(kx)*(ky);
 }

 else if(datatype==3)

 {
	 mat=(void **) malloc(sizeof(float *) *(kx));
	 arrsize=sizeof(float) *(kx)*(ky);
 }
